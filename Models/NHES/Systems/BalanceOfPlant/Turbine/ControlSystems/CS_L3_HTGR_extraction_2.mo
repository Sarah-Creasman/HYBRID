...
model CS_L3_HTGR_extraction_2
  ...
  TRANSFORM.Controls.LimPID FeedPump_PID(
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=-5e-3,
    Ti=30,
    yMax=2*data.mdot_hpt,
    yMin=data.mdot_hpt*0.1)
    annotation (Placement(transformation(extent={{-4,106},{16,126}})));
  Modelica.Blocks.Math.Add add
    annotation (Placement(transformation(extent={{46,100},{66,120}})));
...
equation
  ...
  connect(T_in_set.y,FeedPump_PID. u_s)
    annotation (Line(points={{-79,90},{-16,90},{-16,116},{-6,116}},
                                                 color={0,0,127}));
  connect(actuatorBus.Feed_Pump_Speed,add. y) annotation (Line(
      points={{30,-100},{30,84},{74,84},{74,110},{67,110}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-3,-6},{-3,-6}},
      horizontalAlignment=TextAlignment.Right));
  connect(T_in_set1.y, add.u2) annotation (Line(points={{21,88},{34,88},{34,104},
          {44,104}}, color={0,0,127}));
  connect(sensorBus.Steam_Temperature, FeedPump_PID.u_m) annotation (Line(
      points={{-30,-100},{-10,-100},{-10,80},{6,80},{6,104}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-3,-6},{-3,-6}},
      horizontalAlignment=TextAlignment.Right));
  connect(add.u1, FeedPump_PID.y)
    annotation (Line(points={{44,116},{17,116}}, color={0,0,127}));
annotation(defaultComponentName="changeMe_CS", Icon(graphics),
    experiment(
      StopTime=1000,
      Interval=5,
      __Dymola_Algorithm="Esdirk45a"));
end CS_L3_HTGR_extraction_2;
