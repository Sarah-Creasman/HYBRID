...
model SteamTurbine_L3_HPOFWHsimplified_sec_RemoveReactorVersion
  ...
  TRANSFORM.Fluid.Volumes.Separator moistureSeperator(
    ...
    eta_sep=0.99,
    nPorts_a=1,
    nPorts_b=2) annotation (Placement(transformation(extent={{8,50},{28,70}})));
...
equation
  ...
  connect(moistureSeperator.port_b[1], LPT1.portHP)
    annotation (Line(points={{24,59.75},{32,59.75},{32,60},{38,60}},
                                               color={0,127,255}));
  ...
  connect(LPT1.shaft_b, generator.shaft)
    annotation (Line(points={{58,54},{100,54}}, color={0,0,0}));
  connect(LPT1.portLP, condenser.port_a) annotation (Line(points={{58,60},{70,
          60},{70,-36},{100,-36},{100,-43},{93,-43}}, color={0,127,255}));
  connect(sensor_p3.port, condenser.port_a) annotation (Line(points={{100,68},{
          82,68},{82,60},{70,60},{70,-36},{100,-36},{100,-43},{93,-43}}, color=
          {0,127,255}));
  connect(sensor_p2.port, moistureSeperator.port_b[2]) annotation (Line(points=
          {{64,74},{48,74},{48,60.25},{24,60.25}}, color={0,127,255}));
  connect(Feed_T.port, FWCP.port_b)
    annotation (Line(points={{-90,-60},{-72,-60}}, color={0,127,255}));
  connect(FWCP.port_a, OFWH_2.port_a)
    annotation (Line(points={{-52,-60},{-34,-60}}, color={0,127,255}));
  connect(actuatorBus.Feed_Pump_Speed, FWCP.inputSignal) annotation (Line(
      points={{30,100},{30,36},{-66,36},{-66,-46},{-62,-46},{-62,-52.7}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
...
end SteamTurbine_L3_HPOFWHsimplified_sec_RemoveReactorVersion;
