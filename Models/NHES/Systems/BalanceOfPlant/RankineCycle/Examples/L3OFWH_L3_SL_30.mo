...
//  parameter Modelica.Units.SI.MassFlowRate m_ext=40;
  ...
  NHES.Systems.BalanceOfPlant.RankineCycle.Models.SteamTurbine_L3_HPOFWH_TurbineControled
    BOP(
    ...
    HPT_bypass_valve(m_flow_nominal=600),
    FWCP(use_input=true,  m_flow_nominal=data.mdot_total),
  ...
  TRANSFORM.HeatAndMassTransfer.BoundaryConditions.Heat.HeatFlow
    boundary1(Q_flow=3651.5e6)
    annotation (Placement(transformation(extent={{-84,8},{-72,20}})));
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT bypassdump2(
    ...
    use_p_in=true,
    nPorts=1)
    annotation (Placement(transformation(extent={{140,74},{118,94}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=50e5)
    annotation (Placement(transformation(extent={{154,68},{174,88}})));
...
equation
 ...
 assert(P_ext>bypassdump.medium.p_bar, "Extraction Pressure is below usage pressure",level = AssertionLevel.error);
  ...
  connect(boundary1.port, volume.heatPort)
    annotation (Line(points={{-72,14},{-62,14}}, color={191,0,0}));
  ...
  connect(realExpression1.y, bypassdump2.p_in) annotation (Line(points={{175,78},
          {180,78},{180,92},{142.2,92}}, color={0,0,127}));
  connect(flowCV.port_b, sensor_m_flow1.port_a)
    annotation (Line(points={{18,52},{46,52}}, color={0,127,255}));
  connect(sensor_m_flow1.port_b, bypassdump2.ports[1]) annotation (Line(points={
          {66,52},{74,52},{74,88},{112,88},{112,84},{118,84}}, color={0,127,255}));
  connect(sensor_m_flow1.m_flow, bypassdump1.m_flow_in) annotation (Line(points
        ={{56,55.6},{56,66},{76,66},{76,34},{78,34},{78,-2},{100,-2},{100,-56},{
          78,-56},{78,-48}}, color={0,0,127}));
  connect(stateSensor1.port_b, flowCV.port_a) annotation (Line(points={{-18,26},
          {-4,26},{-4,38},{-2,38},{-2,52}}, color={0,127,255}));
