...
public
  ...
  BaseClasses.SignalSubBus_ActuatorInput actuatorBus
    annotation (Placement(transformation(extent={{30,164},{50,184}})));
  BaseClasses.SignalSubBus_SensorOutput sensorBus
    annotation (Placement(transformation(extent={{-18,164},{2,184}})));
  CS.CS_MSR_PFL changeMe_CS
    annotation (Placement(transformation(extent={{-22,216},{-2,236}})));
  CS.ED_MSR_PFL changeMe_CS1
    annotation (Placement(transformation(extent={{26,218},{46,238}})));
  Modelica.Fluid.Sensors.Temperature inlet_temp(redeclare package Medium =
        Medium_PFL)
    annotation (Placement(transformation(extent={{34,-116},{54,-96}})));
...
protected
  ...
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_T boundary4(
    m_flow=2*3*data_MSR.m_flow_tube,
    T=data_MSR.T_inlet_tube,
    showName=systemTF.showName,
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT boundary1(
    p=data_MSR.p_outlet_tube,
    T=data_MSR.T_outlet_tube,
    showName=systemTF.showName,
    redeclare package Medium = Medium_PCL,
    nPorts=1)                              annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
...
equation
  ...
  connect(pipeFromPHX_PFL.port_b, tee_inlet.port_a[1]) annotation (Line(points={{160,-80},
          {160,-140},{0.25,-140},{0.25,-136}},      color={0,127,255}));
  ...
  connect(boundary4.ports[1], PHX.port_a_shell) annotation (Line(points={{262,-32},
          {164.6,-32},{164.6,-10}}, color={0,127,255}));
  connect(PHX.port_b_shell, boundary1.ports[1]) annotation (Line(points={{164.6,
          10},{226,10},{226,30},{266,30}}, color={0,127,255}));
  ...
  connect(actuatorBus.pump_speed, pump_PFL.in_m_flow) annotation (Line(
      points={{40,174},{40,148},{50,148},{50,135.3}},
      color={111,216,99},
  ...
  connect(outlet_temp.port, plenum_upper.port_b[2]) annotation (Line(points={{28,
          80},{14,80},{14,96},{-0.25,96}}, color={0,127,255}));
  connect(inlet_temp.port, tee_inlet.port_a[2]) annotation (Line(points={{44,-116},
          {44,-136},{-0.25,-136}}, color={0,127,255}));
