...
model
  SteamTurbine_L3_HPOFWHsimplified_sec_RealExpressionswitch_FWHExtractionLPT_PlayingWithPressures
  ...
  TRANSFORM.Fluid.Valves.ValveLinear TBV(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    dp_nominal=200000,
    m_flow_nominal=0) annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={-74,82})));
  ...
  TRANSFORM.Fluid.Valves.ValveLinear HPT_bypass_valve(
    ...
    dp_nominal=50000,
    m_flow_nominal=600) annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={-20,-16})));
  ...
  TRANSFORM.Fluid.Sensors.Pressure sensor_p(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{-134,40},{-154,60}})));
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y=0.4946)
    annotation (Placement(transformation(extent={{30,0},{10,20}})));
  Modelica.Blocks.Logical.Switch switch1
    annotation (Placement(transformation(extent={{66,-20},{46,0}})));
  Modelica.Blocks.Sources.BooleanStep booleanStep1(startTime=2000, startValue=
        true) annotation (Placement(transformation(extent={{172,-12},{152,8}})));
equation
  ...
  connect(actuatorBus.opening_TCV, TCV.opening) annotation (Line(
      points={{30.1,100.1},{-56,100.1},{-56,68},{-54,68},{-54,56},{-66,56}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(sensor_p.port, Steam_T.port)
    annotation (Line(points={{-144,40},{-144,38},{-88,38},{-88,32}},
                                                 color={0,127,255}));
  connect(sensorBus.W_total, sensorW.W) annotation (Line(
      points={{-29.9,100.1},{113,100.1},{113,22}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(sensorBus.Steam_Pressure, sensor_p.p) annotation (Line(
      points={{-30,100},{-84,100},{-84,114},{-160,114},{-160,50},{-150,50}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(generator.port, sensorW.port_a)
    annotation (Line(points={{100,34},{100,32},{102,32}},
                                                 color={255,0,0}));
  connect(port_a_elec, sensorW.port_b)
    annotation (Line(points={{100,0},{100,6},{102,6},{102,12}},
                                                color={255,0,0}));
  ...
  connect(TCV.port_a, port_a_steam) annotation (Line(points={{-76,48},{-76,60},
          {-100,60}},         color={0,127,255}));
  connect(switch1.y, HPT_bypass_valve.opening) annotation (Line(points={{45,-10},
          {-2,-10},{-2,-16},{-12,-16}}, color={0,0,127}));
  connect(realExpression.y, switch1.u3) annotation (Line(points={{9,10},{6,10},
          {6,-22},{42,-22},{42,-28},{68,-28},{68,-18}}, color={0,0,127}));
  connect(actuatorBus.LPT2_BV, switch1.u1) annotation (Line(
      points={{30,100},{30,38},{68,38},{68,-2}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(booleanStep1.y, switch1.u2) annotation (Line(points={{151,-2},{116,-2},
          {116,-10},{112,-10},{112,-12},{92,-12},{92,-14},{78,-14},{78,-10},{68,
          -10}}, color={255,0,255}));
  connect(port_a_steam, LPT1_bypass_valve.port_a) annotation (Line(points={{
          -100,60},{-100,28},{-102,28},{-102,14},{-114,14},{-114,-14},{-38,-14},
          {-38,0},{-42,0}}, color={0,127,255}));
  connect(LPT1_bypass.port_3, HPT_bypass_valve.port_a)
    annotation (Line(points={{-10,50},{-10,-6},{-20,-6}}, color={0,127,255}));
  connect(TCV.port_b, HPT.portHP) annotation (Line(points={{-56,48},{-48,48},{
          -48,60},{-42,60}}, color={0,127,255}));
