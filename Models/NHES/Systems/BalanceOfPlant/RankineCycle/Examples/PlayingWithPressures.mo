...
//  Real eta_CHP "Thermal Cycle Efficiency";
  ...
  Models.SteamTurbine_L3_HPOFWHsimplified_sec_RealExpressionswitch_FWHExtractionLPT_PlayingWithPressures BOP(
    ...
    HPT_bypass_valve(dp_nominal=30000, m_flow_nominal=500),
    pump1(p_nominal=1000000),
    booleanStep1(startTime=1000))
  ...
  NHES.Systems.BalanceOfPlant.RankineCycle.Data.Data_4Turbines data(
    ...
    Tin=550.15,
    Tfeed=500.15,
    d_HPT_in(displayUnit="kg/m3") = 31.80297734,
    d_LPT1_in(displayUnit="kg/m3") = 16.26108896,
    d_LPT2_in(displayUnit="kg/m3") = 6.174594994,
    mdot_total=2019.662582,
    mdot_fh=544.6001198,
    mdpt_HPFH=280,
    mdot_hpt=2019.662582,
    mdot_lpt1=1475.062462,
    mdot_lpt2=1276.797982,
    m_ext=m_ext,
    eta_t=0.9,
    eta_mech=1,
    eta_p=0.8)
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT bypassdump2(
    ...
    use_p_in=true,
    nPorts=1)
    annotation (Placement(transformation(extent={{274,66},{252,86}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=50e5)
    annotation (Placement(transformation(extent={{288,60},{308,80}})));
...
//  eta_CHP=(-BOP.port_a_elec.W-BOP.pump.W-BOP.pump1.W-BOP.FWCP.W+Q_util)/volume.heatPort.Q_flow;
  ...
  connect(boundary1.port,volume. heatPort)
    annotation (Line(points={{-94,22},{-94,24},{-88,24}},
                                                 color={191,0,0}));
  connect(volume.port_b, stateSensor1.port_a) annotation (Line(points={{-82,30},
          {-82,44},{-54,44},{-54,26},{-40,26}}, color={0,127,255}));
  connect(realExpression1.y,bypassdump2. p_in) annotation (Line(points={{309,70},
          {314,70},{314,94},{276.2,94},{276.2,84}},
                                         color={0,0,127}));
  connect(flowCV.port_b,sensor_m_flow1. port_a)
    annotation (Line(points={{152,44},{180,44}},
                                               color={0,127,255}));
  connect(sensor_m_flow1.port_b,bypassdump2. ports[1]) annotation (Line(points={{200,44},
          {246,44},{246,76},{252,76}},                         color={0,127,255}));
  connect(sensor_m_flow1.m_flow, bypassdump1.m_flow_in) annotation (Line(points={{190,
          47.6},{190,60},{102,60},{102,-58},{78,-58},{78,-48}},
                             color={0,0,127}));
  connect(stateSensor1.port_b,flowCV. port_a) annotation (Line(points={{-18,26},
          {2,26},{2,44},{8,44},{8,90},{110,90},{110,44},{132,44}},
                                            color={0,127,255}));
